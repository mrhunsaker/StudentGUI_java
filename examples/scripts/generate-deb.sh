...
# Usage: ./generate-deb.sh [app-version]
# Checks JAVA_HOME, finds the artifact under ../target, and uses jpackage to create a .deb
...
"$JPACKAGE" --type deb --input "$(dirname "$JAR")" --main-jar "$(basename "$JAR")" --name "$APP_NAME" --app-version "$APP_VERSION" --dest "$DEST_DIR" "${@:2}"

echo "DEB artifact created in $DEST_DIR"
